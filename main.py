...
class Othello:
    def __init__(self):
        ...
        self.aiPlayer = AIPlayer(self.grid)
    ...
    def update(self):
        if self.currentPlayer == -1:
            new_time = pygame.time.get_ticks()  # returns the current time in milliseconds since the Pygame module was initialized
            if new_time - self.time >= 100:     # Checks if at least 100 milliseconds have elapsed since the last reference time stored in self.time.
                if not self.grid.findAvailMoves(self.grid.gridLogic, self.currentPlayer):
                    self.gameOver = True
                    return
                
                cell, score = self.aiPlayer.alpha_beta_pruning(self.grid.gridLogic, 5, float('-inf'), float('inf'), -1)
...
class Grid:
    ...
    def loadBackGroundImages(self):
        alpha = 'ABCDEFGHI'
        spriteSheet = pygame.image.load('assets/wood.png').convert_alpha()  #convert_alpha() method is used to convert the image to a format that supports transparency
        imageDict = {}
        for i in range(3):
            for j in range(7):
    ...
    def regenGrid(self, rows, columns):
        ...
        #  initially, all cells are set to 0, indicating empty spaces
        for y in range(rows):
            line = []
    ...
    def endScreen(self):
        if self.GAME.gameOver:
            ...
            endScreenImg.blit(endText, ((320 - endText.get_width()) // 2, 50))  # Centered above the button
            newGame = pygame.draw.rect(endScreenImg, 'White', (80, 160, 160, 80))
            newGameText = self.font.render('Play Again', 1, 'Black')
            endScreenImg.blit(newGameText, ((320 - newGameText.get_width()) // 2, 190))  # Centered horizontally
        return endScreenImg
    ...
    def drawGrid(self, window):
        ...
        if self.GAME.currentPlayer == 1:
            for move, flips in availMoves.items():
                pygame.draw.rect(window, 'White', (80 + (move[1] * 80) + 30, 80 + (move[0] * 80) + 25, 30, 30))
                textImg = self.font.render(str(flips), 1, 'Black')
                window.blit(textImg, (80 + (move[1] * 80) + 40, 80 + (move[0] * 80) + 30))
            if self.GAME.gameOver:
    ...
    def findAvailMoves(self, grid, currentPlayer):
        ...
        for cell in validCells:
            ...
            if len(swapTiles) > 0:
                availableMoves[cell] = len(swapTiles)
...
class Token:
    ...
    def transition(self, transitionImages, tokenImage):
        for i in range(30):
            ...
            self.GAME.draw()
            pygame.time.delay(15)  # Add a delay of 15 milliseconds between frames
...
class AIPlayer:
    ...
    def alpha_beta_pruning(self, grid, depth, alpha, beta, player):
        ...
        if player < 0:
            ...
            for move in availMoves:
                ...
                bMove, value = self.alpha_beta_pruning(newGrid, depth-1, alpha, beta, player *-1)
        ...
        if player > 0:
            ...
            for move in availMoves:
                ...
                bMove, value = self.alpha_beta_pruning(newGrid, depth-1, alpha, beta, player)
